...
// levels from 1 at the root). In level H, which may contain
// less than the maximum possible number of nodes, all the nodes
// are "left-adjusted". This means that in a levelorder tree
// traversal all internal nodes come first, the leaves come
// second, and empty successors (the Ends which are not really
// nodes!) come last.
//
...
// Write a method completeBinaryTree that takes as parameters the
...
object p63 extends App {
  ...
  println(Tree.completeBinaryTree(0, "x"))
  println(Tree.completeBinaryTree(1, "x"))
  println(Tree.completeBinaryTree(2, "x"))
}
